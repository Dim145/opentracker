...
export default defineEventHandler(async (event) => {
  ...
  // values, and non-primitive types up front — et, depuis, la valeur qui ne
  // correspond pas au `type` que l'adaptateur déclare pour ce champ. On
  // valide l'entrant, avant la reprise des secrets : ceux-là ont déjà été
  // validés à leur propre écriture.
  if (parsed.data.userConfig !== undefined) {
    validateChannelConfig(adapter.userFields, parsed.data.userConfig);
  }
