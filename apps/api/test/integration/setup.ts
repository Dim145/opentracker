...
beforeEach(async () => {
  ...
  //
  // `themes` is named for the same reason from the other direction: its only
  // foreign key is `created_by`, nullable and `SET NULL`, so a CASCADE from
  // `users` empties that column and leaves the row behind. A theme surviving
  // into the next test breaks every assertion about which themes are served —
  // and it would do so by ORDER, which is the kind of failure that looks like
  // flakiness.
  await db.execute(
    sql`TRUNCATE TABLE
