...
| `TRACKER_STATS_FLUSH_INTERVAL` | tracker  | `60s`         | How long byte credits are batched in Redis before they are applied to `users`. A member seeding N torrents updates their own row N times per announce interval; the additions are commutative, so nothing requires applying them one at a time. Measured over five minutes of traffic at 1 964 credited announces/s and 50 000 members, at steady state: disabled 113–125 MB of WAL and 589 200 row writes, `60s` 45.8 MB and 226 315, `300s` 7.5 MB and 50 000. The cost is staleness — the counters lag by up to one window. The only gate that reads them is the announce ratio check, which already sees a 60 s-old value from the passkey cache and only repeats once per announce interval per torrent; past 60 s it is the member who notices their own ratio trailing. `0s` restores the per-announce write. |
| `TRACKER_STATS_FLUSH_CHUNK`   | tracker   | `10`          | Members written per transaction during a flush. Do not raise without measuring: past a few dozen, HOT pruning can no longer recycle in-page space, every row migrates and rewrites all seven indexes, and a single-block flush writes MORE WAL than the individual writes it replaces (45 317 rows in one transaction: 19% HOT; in chunks of ten: 99.8%). |
...
| `FEDERATION_REQUIRE_AUDIENCE` | api | `false`                          | Refuse anything that carries only an audience-less signature — **both** the inbound S2S request signature and a relay's countersignature on a record. Each now binds the recipient's `instanceId`, which stops one partner replaying a signed request, or forwarding a countersignature made for somebody else, to another instance. Until every peer runs a version that emits the bound form, an attacker can strip it and downgrade. Turn this on once all partners are upgraded — turned on too early it silently stops accepting a partner's relayed records. |
