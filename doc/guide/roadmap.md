...
- [x] **Login history** — Every attempt to open a session, successful or refused, with the method, the client, and the address as a per-day ordinal. Members read their own; staff read any member's from their profile, where the question of account sharing actually comes up. Failures matter most: there is no per-account lockout on this site, so an attempt spread across addresses meets nothing at the time and this is the only trace it leaves. The daily salt means addresses compare within a day and not across weeks; the ordinal restarts each day so the numbering says that for itself instead of leaving it to a footnote. See [Security](./security.md).
