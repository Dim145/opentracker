...
| Store | Purpose |
| --- | --- |
| **Postgres** | All persistent state — users, torrents, sessions, bonus ledger, notification rows. Schema in `packages/db/src/schema.ts`, applied at API boot by the committed migration chain under `packages/db/src/migrations/`. |
